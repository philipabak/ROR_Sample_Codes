...
require_dependency 'sass/application_stylesheets'
...
class Category < ActiveRecord::Base
  ...
  after_save :publish_application_stylesheet
...
SQL
  ...
  def allowed_tags=(tag_names_arg)
    ApplicationTagging.add_or_create_tags_by_name(self, tag_names_arg, {unlimited: true})
  end
  ...
  def url_with_id
    self.parent_category ? "#{url}/#{self.id}" : "#{Application.base_uri}/c/#{self.id}-#{self.slug}"
  end
  ...
  def publish_application_stylesheet
    ApplicationStylesheets.cache.clear
  end
