...
describe InvitesController do
  ...
  context '.destroy' do
    ...
    it 'requires you to be logged in' do
      ...
      }.to raise_error(Application::NotLoggedIn)
    end
    ...
    context 'while logged in' do
      ...
      it "raises an error when the email cannot be found" do
        expect { delete :destroy, email: 'finn@adventuretime.ooo' }.to raise_error(Application::InvalidParameters)
      end

      it 'raises an error when the invite is not yours' do
        expect { delete :destroy, email: another_invite.email }.to raise_error(Application::InvalidParameters)
      end
  ...
  context '.resend_invite' do
    ...
    context 'while logged in' do
      ...
      it "raises an error when the email cannot be found" do
        expect { post :resend_invite, email: 'first_name@example.com' }.to raise_error(Application::InvalidParameters)
      end

      it 'raises an error when the invite is not yours' do
        expect { post :resend_invite, email: another_invite.email }.to raise_error(Application::InvalidParameters)
      end
  ...
  context '.check_csv_chunk' do
    ...
    context 'while logged in' do
      ...
      let(:resumableIdentifier) { '46-applicationcsv' }
      let(:resumableFilename) { 'application.csv' }
  ...
  context '.upload_csv_chunk' do
    ...
    context 'while logged in' do
      let(:csv_file) { File.new("#{Rails.root}/spec/fixtures/csv/application.csv") }
      let(:file) do
        ActionDispatch::Http::UploadedFile.new({ filename: 'application.csv', tempfile: csv_file })
      end
      let(:resumableChunkNumber) { 1 }
      ...
      let(:resumableIdentifier) { '46-applicationcsv' }
      let(:resumableFilename) { 'application.csv' }
      let(:resumableRelativePath) { 'application.csv' }
